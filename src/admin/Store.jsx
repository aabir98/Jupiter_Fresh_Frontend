...
const ProductImageSlider = ({ product }) => {
  ...
  if (!hasMultiple) {
    return (
      <img src={product.image?.startsWith('/uploads') ? `${API_BASE_URL}${product.image}` : product.image} alt={product.name} style={{ width: '100%', height: '100px', objectFit: 'contain', marginBottom: '12px', borderRadius: '8px' }} />
    );
  }
...
function Store() {
  ...
  const fetchMainCategories = async () => {
    const res = await fetch(`${API_BASE_URL}/api/main-categories`);
  ...
  const handleSaveMainCategory = async (e) => {
    ...
    if (editingMainCategory.id) {
      await fetch(`${API_BASE_URL}/api/main-categories/${editingMainCategory.id}`, { method: 'PUT', body: formData });
    } else {
      await fetch(`${API_BASE_URL}/api/main-categories`, { method: 'POST', body: formData });
    }
  ...
  const handleDeleteMainCategory = async (id) => {
    if (window.confirm("Delete this main category and all its subcategories/products?")) {
      await fetch(`${API_BASE_URL}/api/main-categories/${id}`, { method: 'DELETE' });
      fetchMainCategories();
      fetchCategories();
      fetchProducts();
  ...
  const handleSaveCategory = async (e) => {
    ...
    if (editingCategory.id) {
      await fetch(`${API_BASE_URL}/api/categories/${editingCategory.id}`, { method: 'PUT', body: formData });
    } else {
      await fetch(`${API_BASE_URL}/api/categories`, { method: 'POST', body: formData });
    }
  ...
  const handleSaveProduct = async (e) => {
    ...
    if (editingProduct.id) {
      await fetch(`${API_BASE_URL}/api/products/${editingProduct.id}`, { method: 'PUT', body: formData });
    } else {
  ...
  const handleSaveDeal = async (e) => {
    ...
    if (editingDeal.id) {
      await fetch(`${API_BASE_URL}/api/deals/${editingDeal.id}`, { method: 'PUT', body: formData });
    } else {
      await fetch(`${API_BASE_URL}/api/deals`, { method: 'POST', body: formData });
    }
    setEditingDeal(null);
    fetchDeals();
  };
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px' }}>
                  {currentDeals.map(p => (
                    <div key={p.id} style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column' }}>
                      <img src={p.image?.startsWith('/uploads') ? `${API_BASE_URL}${p.image}` : p.image} alt={p.name} style={{ width: '100%', height: '100px', objectFit: 'contain', marginBottom: '12px' }} />
