...
function OffersEvents() {
  ...
  const handleSaveHub = async (e) => {
    e.preventDefault();
    const url = editingHub ? `${API_BASE_URL}/api/hubs/${editingHub.id}` : `${API_BASE_URL}/api/hubs`;
  ...
  const handleDeleteHub = async (id) => {
    if (window.confirm("Delete this hub?")) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/hubs/${id}`, { method: 'DELETE' });
  ...
  const handleSaveOffer = async (e) => {
    ...
    const url = editingOffer ? `${API_BASE_URL}/api/offers/${editingOffer.id}` : `${API_BASE_URL}/api/offers`;
  ...
  const handleSaveAnnouncement = async (e) => {
    e.preventDefault();
    const url = editingAnnouncement ? `${API_BASE_URL}/api/announcements/${editingAnnouncement.id}` : `${API_BASE_URL}/api/announcements`;
  ...
  const handleToggleFeatured = async (review) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/reviews/${review.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleToggleBannerApproved = async (banner) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/banners/${banner.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
                  {banners.map(banner => (
                    <div key={banner.id} style={{ border: '1px solid #cbd5e1', borderRadius: '12px', padding: '16px', backgroundColor: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
                      <img src={`${API_BASE_URL}${banner.image}`} alt="Banner" style={{ width: '100%', height: '150px', objectFit: 'cover', borderRadius: '8px', marginBottom: '12px', border: '1px solid #e2e8f0' }} />
